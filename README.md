...
-   functions.php: L32 - If a problem occurs with the function “GetUsersByID” (e.g. request returns empty), \$user = [] is not handled.
...
-   app/users/account.php: L59 - You could delete this line as you would overwrite $user in both or either of the following if statements (if their conditions were true). Alternatively, you could move it to the top of the page and remove $user definitions from all if statements.
-   app/users/account.php: L44 - Consider checking for a previously added avatar image, and remove it using unlink().
-   app/users/signup.php: L48 - Consider checking for errors and redirecting accordingly before inserting into the database. Use if(!\$validatedEmail) etc.
-   footer.php: L2 - Calls to a script which doesn’t exist.
-   account.php: L25 - This line triggers an error if $user[“profile-picture”] is not yet set. You could put this image tag inside an if($user[“profile-picture”]) statement to handle this.
